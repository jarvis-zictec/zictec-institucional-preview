...
def talk_url(context):
    msg = f"Olá, vim pelo site da ZICTEC e quero falar com um especialista sobre {context}."
    return "https://wa.me/554732300435?text=" + quote(msg)

def context_from_title(title):
    base = re.sub(r"\s+—\s+.*$", "", title).strip()
    if not base or base.upper() == "ZICTEC":
        return "consultoria técnica e regulatória para operações de voz"
    return base

def header(active, context):
    nav = []
...
def layout(title, desc, active, body):
    context = context_from_title(title)
    return f'''<!doctype html><html lang="pt-BR"><head>
  <meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
  <title>{title}</title><meta name="description" content="{desc}">
  <link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="styles.css?v=20260619-polish1">
</head><body>{header(active, context)}<main>{body}</main>{footer()}</body></html>'''
...
def cta(title='Vamos avaliar sua operação de voz?', text='A próxima conversa pode começar por diagnóstico técnico, priorização regulatória ou uma frente específica como Origem Verificada, STFC Tools ou SBC.', primary='Agendar diagnóstico'):
    return f'''<section><div class="wrap"><div class="cta"><div><h2>{title}</h2><p>{text}</p></div><div class="cta-actions"><a class="btn secondary" href="https://www.calendly.com/zictec/reuniao" target="_blank" rel="noopener">{primary}</a><a class="btn secondary" href="{talk_url(title)}" target="_blank" rel="noopener">Falar com especialista</a><a class="btn secondary" href="https://shop.zictec.com.br/" target="_blank" rel="noopener">Loja ZICTEC</a></div></div></div></section>'''
